...
def parsePigArray(file, count = 0, schema=list(), indexCol="", otherCols = list(), asLists=False):
    fileName = file.name
    ...
    for fieldId in range(0,0  if len(schema)==0 else len(schema['fields'])):
        if len(otherCols)>0 and (str(schema['fields'][int(fieldId)]['name']) != "null") and (str(schema['fields'][int(fieldId)]['name'])) not in otherCols:
            continue;
            ...
            if isFlat and len(typeList) > 0 :
                for ind in range(0,len(typeList)):
                    if (parsedLine2[fieldListsArr[ind]] != "" ):
                        parsedLine.append( int(parsedLine2[fieldListsArr[ind]]) if typeList[ind] == 15 else parsedLine2[fieldListsArr[ind]])
            else:
    ...
    if len(otherCols)>0:
        if type(otherCols[0]) == int:
            ...
            for fieldId in range(0,fieldsLen):
                if (str(schema['fields'][int(fieldId)]['name']) != "null") and (str(schema['fields'][int(fieldId)]['name'])) not in otherCols:
                    continue;
                fieldListsArr.append(fieldId)
    res = list()
...
def saveFields(param_list):
    ...
    lineSplitted= param_list[0].split() if (splitter==" ") else param_list[0].split(splitter)
    ...
    if (len(fieldListArr)>0):
       fieldLists=fieldListArr
        ...
        for fieldId in range(0,fieldsLen):
            if (len(otherCols)>0 and str(param_list[1]['fields'][int(fieldId)]['name']) != "null") and (str(param_list[1]['fields'][int(fieldId)]['name'])) not in otherCols:
                continue;
    ...
    for fieldId in fieldLists:
        ...
        if (fieldDesc['type']=="15" or fieldDesc['type']=="10"):
            if asLists:
                val = lineSplitted[fieldId] if (fieldDesc['type']!="15") else int(lineSplitted[fieldId])
                if fieldsLenActual>1:
                    res.append(val)
                else:
                    res = val
            else:
                res[str(param_list[1]['fields'][int(fieldId)]['name'])] = lineSplitted[fieldId] if (fieldDesc['type']!="15") else int(lineSplitted[fieldId])
        if (fieldDesc['type']=="120") :
            close =  getEndOfBlock([lineSplitted[fieldId]])-1;
            if fieldsLenActual == 1:
                res = saveFields([lineSplitted[fieldId][1:close],fieldDesc['schema'], " ",otherCols, asLists, list()])
            else:
                if asLists:
                    res.append(saveFields([lineSplitted[fieldId][1:close],fieldDesc['schema'], " ",otherCols, asLists, list()]))
                else:
                    res[str(param_list[1]['fields'][int(fieldId)]['name'])] = saveFields([lineSplitted[fieldId][1:close],fieldDesc['schema'], " ",otherCols, asLists, list()])
            
        if (fieldDesc['type']=="110"):
            if fieldsLenActual == 1:
                
                res = saveArray(lineSplitted[fieldId],fieldDesc['schema'],otherCols, asLists)

            else:
                if asLists:
                    res.append(saveArray(lineSplitted[fieldId],fieldDesc['schema'],otherCols, asLists))
                else:
                    res[str(param_list[1]['fields'][int(fieldId)]['name'])]=saveArray(lineSplitted[fieldId],fieldDesc['schema'],otherCols, asLists)
...
def parseFolderBySchema(folderName,  linesCount = 0, fileDesc="", indexCol="", otherCols = list(), isLists = False):
    ...
    return (parsePigArray(fileDesc,linesCount,schema,indexCol,otherCols,isLists),fileDesc)
